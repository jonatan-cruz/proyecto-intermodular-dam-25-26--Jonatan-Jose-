...
# Importar configuración (si usas el archivo config.py)
try:
    from ..config import (
        JWT_SECRET_KEY, 
        JWT_ALGORITHM, 
...
class SecondMarketAuthController(http.Controller):
    """Controlador de autenticación para Second Market"""
    ...
    def login(self, **kwargs):
        ...
        try:
            ...
            return {
                ...
                'error_detail': str(e) if request.env.user.has_group('base.group_system') else None
            }

    @http.route('/api/auth/register', type='json', auth='public', methods=['POST'], csrf=False, cors='*')
    ...
    @http.route('/api/auth/verify', type='json', auth='public', methods=['POST'], csrf=False, cors='*')
    def verify_token(self, **kwargs):
        ...
        try:
            ...
            if not token:
                return {
                    'success': False,
                    'message': RESPONSE_MESSAGES.get('TOKEN_MISSING', 'Token no proporcionado'),
            ...
            try:
                ...
                return {
                    'success': False,
                    'message': RESPONSE_MESSAGES.get('TOKEN_EXPIRED', 'Token expirado'),
                    'error_code': ERROR_CODES.get('TOKEN_EXPIRED', 'TOKEN_EXPIRED')
                }
    ...
    @http.route('/api/auth/refresh', type='json', auth='public', methods=['POST'], csrf=False, cors='*')
...
# ============================================
# FUNCIÓN AUXILIAR PARA OTROS CONTROLADORES
# ============================================
...
def verify_jwt_token(token):
    ...
    Uso en otros controladores:
        from odoo.addons.second_market_api.controllers.auth_controller import verify_jwt_token
    ...
    try:
        ...
        if user.exists() and user.activo:
            return {
                'user_id': user.id,
                'id_usuario': user.id_usuario,
                'login': user.login,
...
def get_token_from_request():
    ...
    if hasattr(request, 'jsonrequest') and request.jsonrequest:
        return request.jsonrequest.get('token')
